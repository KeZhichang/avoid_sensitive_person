# -*- coding: utf-8 -*-
"""
Created on Wed Jun  6 10:17:47 2018

@author: zhichang
"""
#敏感人物检测
import face_recognition
from sensitive_person import sensitive_image_encoding

#载入敏感图片（感觉可以换成载入图片的特征数据会好一点）
...
#]
#对比用户图片与敏感人物图片
results = face_recognition.compare_faces(sensitive_image_encoding, unknown_face_encoding)
...
class Faceswapper():
    ...
    实例化时载入多个头照片资源
    '''
    def __init__(self,heads_list=[],predictor_path="/home/zhichang/dlib-19.12/shape_predictor_68_face_landmarks.dat"):
        '''
        head_list:
    ...
    def get_landmarks(self,im,fname,n=1):
        ...
        if len(rects) < 0:
            raise TooManyFaces('没有检测到人脸 '+fname)
            quit()
        if len(rects) > n:
            raise NoFace('人脸太多'+fname)
            quit()
...
# =============================================================================
    ...
    def swap(self,head_name,face_path):
        ...
        face_path:
            脸来源的图像路径名
        '''
        im_head1, landmarks_head1 = self.heads[head_name];
        im_face, landmarks_face = self.read_and_mark(face_path);
        im_head,landmarks_head,im_face,landmarks_face=self.resize(im_head1,landmarks_head1,im_face,landmarks_face)
...
if __name__=='__main__':
    ...
    head = '/home/zhichang/桌面/code/biden.jpg'
    face = image
    out = 'output.jpg'
    swapper=Faceswapper([head])#head就是背景图片
    output_im=swapper.swap(os.path.split(head)[-1],face)#face就是脸部图片
    swapper.save(out,output_im)#output是输出的图片
